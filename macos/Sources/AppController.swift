...
final class AppController: NSObject, ObservableObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let menu = NSMenu()
        let actions: [(String, Selector)] = [("Настройки…", #selector(showSettings)), ("Проверить уведомление", #selector(testCurrentSettings)), ("Подключиться заново", #selector(connect)), ("Пауза", #selector(togglePause))]
        for (title, selector) in actions {
            ...
            if title == "Пауза" { pauseItem = entry }
        }
        menu.addItem(.separator())
        let quit = NSMenuItem(title: "Выход", action: #selector(quitApp), keyEquivalent: "q"); quit.target = self; menu.addItem(quit)
    ...
    @objc func showSettings() {
        ...
        window.title = "Настройки — Codex Notifier"; window.isReleasedWhenClosed = false
    ...
    func save(_ value: Settings) throws {
        ...
        if SMAppService.mainApp.status == .requiresApproval { monitorStatus = "Подтверди автозапуск в Системных настройках → Основные → Объекты входа." }
    }
    private func title(_ thread: String) -> String {
        guard let data = try? Data(contentsOf: ReplyData.home.appendingPathComponent("session_index.jsonl")), let text = String(data: data, encoding: .utf8) else { return "Ответ в Codex готов" }
        var title = "Задача " + String(thread.prefix(8))
    ...
    private func receive(_ event: SessionEvent) {
        ...
        queue.removeAll { $0.thread == event.thread }
        queue.append(PendingNotice(thread: event.thread, title: title(event.thread) + (event.kind == "turn_aborted" ? " — остановлено" : "")))
        ...
        if displaySettings.autoOpen {
            countdown = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
                ...
                if self.seconds == 0 {
                    ...
                    if notice.thread == nil { self.replyStatus = "Тест завершён. Настоящий чат не открыт."; self.resizePanel() }
    ...
    func test(_ options: Settings) {
        if let notice = current, notice.thread != nil { queue.insert(notice, at: 0) }
        display(PendingNotice(thread: nil, title: "Визуальный стиль уведомлений"), options: options)
    }
    func reply(_ text: String) {
        guard !replyInProgress, let notice = current else { return }
        countdown?.invalidate(); countdown = nil
        guard let thread = notice.thread else { replyStatus = "\(text) · демонстрация, сообщение не отправлено"; resizePanel(); return }

...
sealed class Notifier : ApplicationContext {
    ...
    void BeginQuickReply(string text) {
        ...
        if(settings.ReplyMode=="send") {
            ...
            var target=popup;
            popup.SetReplyStatus("Отправляю сообщение в фоне…",true);
            System.Threading.ThreadPool.QueueUserWorkItem(delegate {
                ...
                target.Window.Dispatcher.BeginInvoke(new Action(delegate {
                    ...
                    else {popup.Show();popup.SetReplyStatus("Отправка не подтверждена. Открой чат и проверь перед повтором.",true);}
                }));
            });
            return;
        }

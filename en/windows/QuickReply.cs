...
static class QuickReply {
    ...
    public static string Link(string thread,string text) {
        ...
        if(Array.IndexOf(Texts,text)<0)throw new ArgumentException("Unknown quick reply.");
    ...
    public static uint BeginInputWatch() {
        ...
        if(keyboardHook==IntPtr.Zero || mouseHook==IntPtr.Zero) {StopInputWatch();throw new InvalidOperationException("Could not verify input activity.");}

...
sealed class Popup : IDisposable {
    ...
    public Popup(string title,Action open,Action ack,AppSettings options,Action<string> reply=null) {
        ...
        DesktopUi.Get<TextBlock>(window,"ReplyHint").Text=settings.ReplyMode=="send"?"Quick reply · send immediately":"Quick reply · insert for review";
        string[] replyButtons={"ReplyYes","ReplyNext","ReplyDo"};
        for(int i=0;i<replyButtons.Length;i++) {string text=QuickReply.Texts[i];var button=DesktopUi.Get<Button>(window,replyButtons[i]);button.Click+=delegate {if(reply!=null)reply(text);else SetReplyStatus("Test: "+text,false);};}
    ...
    public void UpdateSeconds(int n) {
        ...
        string text=!settings.AutoOpen?"Open manually":n>0?"Opening task in: "+n+" sec.":"Codex opened · click Got it";
...
sealed class SettingsDialog {
    ...
    public SettingsDialog(AppSettings current,Action<AppSettings> onSave) {
        ...
        if(current.SetupComplete) { Get<TextBlock>("Eyebrow").Text="CODEX NOTIFIER  /  SETTINGS"; Get<Button>("SaveSettings").Content="Save"; }
        ...
        foreach(string name in new[]{"PreviewOpen","PreviewMiniOpen"}) Get<Button>(name).Click+=delegate { Get<TextBlock>("PreviewStatus").Text="Task opened · preview"; };
        Get<Button>("PreviewAck").Click+=delegate { Get<TextBlock>("PreviewStatus").Text="Notification dismissed · opening cancelled"; };
        string[] previewReplies={"PreviewYes","PreviewNext","PreviewDo"};for(int i=0;i<previewReplies.Length;i++) {string text=QuickReply.Texts[i];Get<Button>(previewReplies[i]).Click+=delegate {Get<TextBlock>("PreviewStatus").Text=(draft.ReplyMode=="send"?"Send: ":"Insert: ")+text+" · preview";};}
        ...
        previewTimer.Tick+=delegate {
            ...
            if(left==0) { StopTest(); Get<TextBlock>("PreviewStatus").Text="Test finished. A real notification would open Codex."; }
        };
    ...
    void Refresh() {
        ...
        Get<TextBlock>("PreviewCountdown").Text=draft.AutoOpen?"Opening task in: "+draft.DelaySeconds+" sec.":"Open manually";
    ...
    void Test() {
        StopTest();
        testPopup=new Popup("Notification test",delegate {StopTest();Get<TextBlock>("PreviewStatus").Text="Task opened · preview";},delegate {StopTest();Get<TextBlock>("PreviewStatus").Text="Notification dismissed · opening cancelled";},draft);
        testPopup.Show(); if(draft.Sound)System.Media.SystemSounds.Exclamation.Play();
        Get<TextBlock>("PreviewStatus").Text="Test notification shown near the clock.";

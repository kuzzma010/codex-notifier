...
sealed class Notifier : ApplicationContext {
    ...
    public Notifier(bool demo) {
        ...
        tray = new NotifyIcon { Icon=Icon.ExtractAssociatedIcon(Application.ExecutablePath), Text="Codex Notifier — enabled", Visible=true };
        ...
        menu.Items.Add("Show notification", null, delegate { if (popup != null) popup.Show(); });
        menu.Items.Add("Test notification", null, delegate { Add("", "demo-"+DateTime.UtcNow.Ticks, "Notification test — opens Codex"); });
        pause = new ToolStripMenuItem("Pause", null, delegate { paused=!paused; pause.Checked=paused; tray.Text=paused ? "Codex Notifier — paused" : "Codex Notifier — enabled"; if(paused) Clear(); }); menu.Items.Add(pause);
        menu.Items.Add("Quit", null, delegate { ExitThread(); }); tray.ContextMenuStrip=menu;
    ...
    string Title(string id) {
        try {
            string result="Task " + id.Substring(0,8);
            ...
            return result;
        } catch { return "Your Codex reply is ready"; }
    }
    ...
    void BeginQuickReply(string text) {
        ...
        try {
            ...
            popup.SetReplyStatus(settings.ReplyMode=="send"?"Opening task and preparing to send…":"Opening task and inserting text…",true);
            Process.Start(new ProcessStartInfo(QuickReply.Link(thread,text)){UseShellExecute=true});
        } catch(Exception e) {pendingReply=null;popup.SetReplyStatus("Could not prepare the reply. Open the task manually.",false);Log("quick-reply-error "+e.GetType().Name);}
    }
    ...
    void PollQuickReply() {
        ...
        if(DateTime.UtcNow>pending.Deadline) {
            pendingReply=null;popup.SetReplyStatus(pending.Sent?"Sending was not confirmed. Check the task before trying again.":"Auto-send did not complete. Review the text and send manually.",pending.Sent);Log("quick-reply-timeout stage="+pending.Stage);return;
        }
        if(pending.Sent)return;
        if(settings.ReplyMode=="send" && QuickReply.InputStamp()!=pending.Input) {
            pendingReply=null;Log("quick-reply-cancelled user-input");popup.SetReplyStatus("Sending cancelled because of new input. Check the task draft.",false);return;
        }
        ...
        string reason;
        if(QuickReply.SubmitToFocusedComposer(hwnd,pending.Input,pending.Text,out reason)) {QuickReply.StopInputWatch();pending.Sent=true;pending.Deadline=DateTime.UtcNow.AddSeconds(10);popup.SetReplyStatus("Waiting for sending confirmation…",true);Log("quick-reply-submit-requested");}
    ...
    void OpenCurrent() { if(queue.Count==0)return; if(Open(queue[0].Thread))Acknowledge();else if(popup!=null)popup.SetReplyStatus("Could not open the task. The notification was kept.",false); }
